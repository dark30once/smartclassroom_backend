...
@mqtt.on_connect()
def handle_connect(client, userdata, flags, rc):# Push context for on_connect callback
...
def handle_mqtt_message(client, userdata, message):
    # Push context for on_message callback
    with app.app_context(): 
        da = dict(
            topic=message.topic,
...
def reload_mqtt():
    ...
    # This function is now called from within an app_context() block, so no extra 'with' is needed here, 
    # but the caller MUST ensure the context is active.
    room_status = RoomStatus.query.all()
...
def handle_logging(client, userdata, level, buf):
    # This also needs context if it uses Flask app config
    with app.app_context(): 
        if app.config['debug'] and level==MQTT_LOG_DEBUG or level!=MQTT_LOG_DEBUG:
            print(level, buf)

if __name__ == '__main__':
    # When running the app directly, the app context is typically handled by app.run(), 
    # but background threads initiated by app.run() might still need the 'with' block as above.
